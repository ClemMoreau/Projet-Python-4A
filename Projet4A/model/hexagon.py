        ...
        for i in range(-2, (self.nb_poly_per_column + 1)):
            ...
            for j in range(-2, (self.nb_poly_per_line + 1)):
                ...
                if (j % 2 == 0):
                    polygon = QtGui.QPolygon()
                    # TOP CENTER POINT
                    polygon << QtCore.QPoint((TRIANGLE_CONST) * self.length + ordinate * HEXAGON_CONST,
                                             abscissa * 1.5)
                    # TOP LEFT POINT
                    polygon << QtCore.QPoint(ordinate * HEXAGON_CONST,
                                             0.5 * self.length + abscissa * 1.5)
                    # BOTTOM LEFT POINT
                    polygon << QtCore.QPoint(ordinate * HEXAGON_CONST,
                                             1.5 * (self.length + abscissa))
                    # BOTTOM CENTER POINT
                    polygon << QtCore.QPoint((TRIANGLE_CONST) * self.length + ordinate * HEXAGON_CONST,
                                             2 * self.length + 1.5 * abscissa)
                    # BOTTOM RIGHT POINT
                    polygon << QtCore.QPoint(HEXAGON_CONST * self.length + HEXAGON_CONST * ordinate,
                                             1.5 * (self.length + abscissa))
                    # TOP RIGHT POINT
                    polygon << QtCore.QPoint(HEXAGON_CONST * self.length + HEXAGON_CONST * ordinate,
                                             self.length * 0.5 + abscissa * 1.5)
                    polygon_list.append(polygon)

                    polygon2 = QtGui.QPolygon()
                    # TOP CENTER POINT
                    polygon2 << QtCore.QPoint(ordinate * HEXAGON_CONST,
                                              1.5 * (self.length + abscissa))
                    # TOP LEFT POINT
                    polygon2 << QtCore.QPoint((ordinate * HEXAGON_CONST) - HEXAGON_CONST * self.length * 0.5,
                                              2 * self.length + abscissa * 1.5)
                    # BOTTOM LEFT POINT
                    polygon2 << QtCore.QPoint((ordinate * HEXAGON_CONST) - HEXAGON_CONST * self.length * 0.5,
                                              3 * self.length + abscissa * 1.5)
                    # BOTTOM CENTER POINT
                    polygon2 << QtCore.QPoint(ordinate * HEXAGON_CONST,
                                              3.5 * self.length + abscissa * 1.5)
                    # BOTTOM RIGHT POINT
                    polygon2 << QtCore.QPoint(TRIANGLE_CONST * self.length + ordinate * HEXAGON_CONST,
                                              abscissa * 1.5 + 3 * self.length)

                    # TOP RIGHT POINT
                    polygon2 << QtCore.QPoint(TRIANGLE_CONST * self.length + ordinate * HEXAGON_CONST,
                                              abscissa * 1.5 + 2 * self.length)
